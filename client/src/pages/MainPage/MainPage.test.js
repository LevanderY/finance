...
describe('Main page', () => {
	...
	it('Show tickers list', () => {
		const { getByTestId } = customTestRender(<MainPage />, {
			preloadedState: data,
		});

		expect(getByTestId(`tickers-${tickers[0].ticker}`)).toHaveTextContent(
			'AAPL'
		);

		expect(getByTestId(`tickers-${tickers[3].ticker}`)).toHaveTextContent(
			'AMZN'
		);
	});
});